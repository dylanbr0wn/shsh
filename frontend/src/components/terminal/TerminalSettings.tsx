...
export function TerminalSettings({
  ...
  const focusedChannelId = useAtomValue(focusedChannelIdAtom)
  ...
  const activeChannelId = channelIdProp ?? focusedChannelId

  // Resolve host: use hostIdProp directly if provided, otherwise derive from workspace
  const host = (() => {
    if (hostIdProp) {
      return hosts.find((h) => h.id === hostIdProp)
    }
    const activeWs = workspaces.find((w) => w.id === activeWorkspaceId)
    const focusedLeaf = activeWs?.focusedPaneId
      ? collectLeaves(activeWs.layout).find((l) => l.paneId === activeWs.focusedPaneId)
      : null
    return hosts.find((h) => h.id === focusedLeaf?.hostId)
  })()
