...
import { AddTerminalProfile, UpdateTerminalProfile, DeleteTerminalProfile } from '../../../wailsjs/go/main/App'
...
export function TerminalProfilesModal() {
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        ...
        setProfiles(prev => prev.map(p => p.id === selectedId ? (updated as unknown as TerminalProfile) : p))
  ...
  const hasChanges = isNew || (selected && (
    ...
    <AlertDialog open={confirmDelete} onOpenChange={setConfirmDelete}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This profile will be removed. Hosts using it will revert to their group or global defaults.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
    ...
    <Dialog open={isOpen} onOpenChange={open => !open && close()}>
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="flex min-h-[420px]">
          {/* Profile list */}
          <div className="border-r w-52 flex flex-col shrink-0">
            <div className="flex-1 overflow-y-auto py-2">
              {profiles.length === 0 && !isNew && (
                <p className="text-muted-foreground text-xs px-4 py-3">No profiles yet</p>
              )}
              {profiles.map(p => (
                <Button
                  key={p.id}
                  ...
                  className={cn(
                    ...
                    selectedId === p.id && !isNew && 'bg-accent text-accent-foreground font-medium'
                  )}
                >
                  {p.name}
                </Button>
              ...
              {isNew && (
                <div className="px-4 py-2 text-sm bg-accent text-accent-foreground font-medium">
                  New Profile
                </div>
              )}
            </div>
          ...
          {(selectedId || isNew) ? (
            <div className="flex-1 flex flex-col">
              <div className="flex-1 overflow-y-auto px-6 py-4">
                ...
                {/* Name */}
                <Field>
                  <FieldLabel htmlFor="tp-name" className="text-xs">Profile Name</FieldLabel>
                  <Input
                    id="tp-name"
                ...
                {/* Color theme */}
                <Field>
                  <FieldLabel htmlFor="tp-theme" className="text-xs">Color Theme</FieldLabel>
                  <Select value={form.colorTheme} onValueChange={v => update('colorTheme', v)}>
                ...
                <Field>
                  <div className="flex items-center justify-between">
                    <FieldLabel htmlFor="tp-fontsize" className="text-xs">Font Size</FieldLabel>
                ...
                {/* Cursor style */}
                <Field>
                  <FieldLabel id="tp-cursor-label" className="text-xs">Cursor Style</FieldLabel>
                  <ToggleGroup
                    type="single"
                  ...
                  >
                    {CURSOR_STYLES.map(style => (
                      <ToggleGroupItem key={style} value={style} className="flex-1 text-xs capitalize h-7">
                        {style}
                      </ToggleGroupItem>
                    ))}
                  </ToggleGroup>
                ...
                <Field orientation="horizontal">
                  <FieldLabel className="text-xs" htmlFor="tp-blink">Cursor Blink</FieldLabel>
                  <Switch
                    id="tp-blink"
                ...
                {/* Scrollback */}
                <Field>
                  <FieldLabel id="tp-scrollback-label" className="text-xs">Scrollback Lines</FieldLabel>
                  <ToggleGroup
                    type="single"
                  ...
                  >
                    {SCROLLBACK_OPTIONS.map(n => (
                      <ToggleGroupItem key={n} value={String(n)} className="text-xs h-7">
                        {n.toLocaleString()}
                      </ToggleGroupItem>
                    ))}
                  </ToggleGroup>
